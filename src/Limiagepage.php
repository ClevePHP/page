...
class Limiagepage
{
     ...
     *            分页跳转的参数
     */
    public function _init($totalRows, $listRows = 20, $parameter = array())
    {
        /* 基础设置 */
        ...
        } elseif (! $requestData && $postData) {
            $requestData = $postData;
            unset($postData);
        }
        $rawContent = $this->request->rawContent();
        if ($rawContent) {
            $jsonData = $rawContent = json_decode($rawContent, TRUE);
            if ($jsonData) {
                $rawContent = $jsonData;
            }
        }
        if ($requestData && $rawContent) {
            $requestData = array_merge($requestData, $rawContent);
        } elseif (! $requestData && $rawContent) {
            $requestData = $rawContent;
            unset($rawContent);
        }

        $this->parameter = empty($parameter) ? $requestData : $parameter;
        $this->nowPage = empty($requestData[$this->p]) ? 1 : intval($requestData[$this->p]);
